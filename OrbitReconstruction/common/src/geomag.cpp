...
//static void clear_field(BField *const bfield);
                         ...
                         const double sdate)
{
  ...
  warn_H_strong_val = 99999.0;
	...
	shval3(coordSys, latitude, longitude, alt, nmax, 3,IEXT, EXT_COEFF1, EXT_COEFF2, EXT_COEFF3, gha, ghb,&(bfield->x), &(bfield->y), &(bfield->z));
	dihf(bfield->x, bfield->y, bfield->z,&(bfield->d), &(bfield->i), &(bfield->h), &(bfield->f));
	shval3(coordSys, latitude, longitude, alt, nmax, 4,IEXT, EXT_COEFF1, EXT_COEFF2, EXT_COEFF3, gha, ghb,&xtemp, &ytemp, &ztemp);
