...
public:
      ...
      std::string ToString() const
  {
    ...
    ss << altitude; //Altitude in Kilometers (plus letter DE to be read in Geomag
    ss << " " <<Util::RadiansToDegrees(latitude); //Latitude
    ss << " " <<Util::RadiansToDegrees(longitude); //Longitude
    return ss.str();
  }
